...
import { ArrowRight, CalendarDays, CheckCircle2, ClipboardCheck, Sparkles, User } from 'lucide-react'
import { useNavigate } from 'react-router-dom'
import { useOnboardingState } from '../../hooks/useOnboardingState'
import type { OnboardingStep, PrimaryGoalOption, UserTypeOption } from '../../types/onboarding'
...
const stepMap: Record<OnboardingStep, { title: string; subtitle: string }> = {
  ...
  notifications: {
    title: 'Stay on track.',
    subtitle: 'Enable notifications so BuildLog can remind you when you have not shared your progress.',
  },
  complete: {
...
function isStepComplete(step: OnboardingStep) {
...
export default function OnboardingPage() {
  ...
  const currentStepNumber = useMemo(() => {
    const order: OnboardingStep[] = ['welcome', 'userType', 'primaryGoal', 'reminder', 'notifications']
  ...
  const handleNotificationRequest = async () => {
    ...
    try {
      ...
      if (permission === 'denied') {
        setNotificationError('You can still log progress without notifications. We will keep your setup ready.')
      }
    } catch {
      setNotificationPermission('default')
      setNotificationError('Something went wrong while requesting permission. You can still continue.')
    }
  }
    ...
    <main className="min-h-screen bg-white text-[#0F172A] selection:bg-[#FFD166] selection:text-[#0F172A] font-sans flex items-center justify-center px-6 py-12">
      <div className="w-full max-w-175 space-y-10">
        <div className="flex flex-col items-center gap-3 text-center">
          <ProgressIndicator
            currentIndex={currentSection === 'complete' ? stepCount : currentStepNumber}
            total={stepCount}
            label={formatStepLabel(state.step, currentStepNumber - 1, stepCount)}
          />
        ...
        >
            ...
            <section className="space-y-8 rounded-3xl border border-[#E2E8F0] bg-[#F8FAFC] p-8 shadow-[0_4px_12px_rgba(15,23,42,0.06)]">
              <div className="space-y-4">
                <p className="text-[#475569] text-lg leading-relaxed max-w-xl mx-auto">
                  BuildLog turns your daily progress into content worth sharing. We'll personalize your experience in less than a minute.
                </p>
                <div className="grid gap-4 sm:grid-cols-3">
                  <div className="rounded-3xl border border-[#E2E8F0] bg-white p-5 shadow-sm">
                    <p className="text-sm uppercase tracking-[0.24em] text-[#64748B]">Work Log</p>
                    <p className="mt-4 font-semibold text-[#0F172A]">Record your progress</p>
                  </div>
                  <div className="rounded-3xl border border-[#E2E8F0] bg-white p-5 shadow-sm">
                    <p className="text-sm uppercase tracking-[0.24em] text-[#64748B]">Content Draft</p>
                    <p className="mt-4 font-semibold text-[#0F172A]">Save a ready-to-share post</p>
                  </div>
                  <div className="rounded-3xl border border-[#E2E8F0] bg-white p-5 shadow-sm">
                    <p className="text-sm uppercase tracking-[0.24em] text-[#64748B]">Published</p>
                    <p className="mt-4 font-semibold text-[#0F172A]">Share your work with confidence</p>
                  </div>
                </div>
              </div>
            ...
            <section className="space-y-8 rounded-3xl border border-[#E2E8F0] bg-[#F8FAFC] p-8 shadow-[0_4px_12px_rgba(15,23,42,0.06)]">
              <div className="grid gap-6 sm:grid-cols-[1.15fr_0.85fr]">
                <label className="space-y-3">
                  <span className="block text-sm font-semibold text-[#0F172A]">Reminder time</span>
                  <input
                    type="time"
                    value={state.reminderTime}
                    onChange={(event) => saveReminder(event.target.value, timezoneInput)}
                ...
                <label className="space-y-3">
                  <span className="block text-sm font-semibold text-[#0F172A]">Timezone</span>
                  <div className="grid gap-2">
                    <select
                      ...
                      className="w-full rounded-2xl border border-[#E2E8F0] bg-white px-4 py-3 text-sm text-[#0F172A] focus:border-[#FF6B35] focus:outline-none focus:ring-2 focus:ring-[#FF6B35]/20"
                    >
                      <option value={state.timezone}>{state.timezone} (auto)</option>
                    ...
                    <span className="text-sm text-[#64748B]">We will respect this schedule only when you haven&apos;t logged progress.</span>
                  </div>
                </label>
              </div>
          ...
          {state.step === 'notifications' && (
            <section className="space-y-6 rounded-3xl border border-[#E2E8F0] bg-[#F8FAFC] p-8 shadow-[0_4px_12px_rgba(15,23,42,0.06)]">
              <NotificationPreviewCard permission={state.notificationPermission} />
              {notificationError ? (
